...
def train_gcn(model, data, optimizer, epochs, model_save_path=None, device='cpu'):
    print(f"开始在 {device} 上训练干净的GCN模型...")
    model.to(device)
    data = data.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch == 1 or epoch % 5 == 0 or epoch == epochs:
            print(
                f"[Clean GCN] epoch {epoch}/{epochs} "
...
def test_gcn(model, data, device='cpu'):
    model.to(device)
    data = data.to(device)
    model.eval()
    with torch.no_grad():
...
def train_clean_GCN(data, dataset_name='dataset', device='cpu', epochs=200, hidden_channels=16, dropout=0.5, lr=0.01, weight_decay=5e-4, output_dir='./clean_models'):
    ...
    model_filename = f"gcn_{str(dataset_name).lower()}_clean.pth"
    model_save_path = os.path.join(output_dir, model_filename)
    trained_model, best_val_accuracy = train_gcn(model, data, optimizer, int(epochs), model_save_path, device)
    test_accuracy = test_gcn(trained_model, data, device)
...
train_clean_gcn = train_clean_GCN
train_clean_model = train_clean_GCN
